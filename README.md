<h1 align="center"> CeciAdminMT </h1>

> Status do Projeto: Concluído :heavy_check_mark:

## Descrição do Projeto
<p align="justify"> O projeto CeciAdminMT é um projeto que possui arquitetura e funcionalidade básicas para o início de novos projetos cujo objetivo seja administrar aplicações multitenant, ou seja essa arquitetura é a responsável por manter o controle de tenants ativos e novos tenants. 

Esse projeto é um complemento do projeto [CeciMT](https://github.com/pablofsilva91/ceci-mt), que contém arquitetua e funcionalidades básicas para o início de novos projetos multitenant, porém o objetivo é apenas o controle de leitura e gravação dos tenants, dependendo assim de uma outra aplicação para a gestão dos tenants, como por exemplo o projeto CeciAdminMT. 
...
- Tenants
    - Armazemamento persistente de empresas (criar, ler, atualizar e excluir)
...
:dart: Manter cadastro de empresas (tenants) 
...
```
git clone https://github.com/pablofsilva91/ceci-admin-mt.git
```
...
1. Criando tabelas com a migração inicial do projeto
    ...
    ```
    dotnet ef database update --project CeciAdminMT.Infra.Data --startup-project CeciAdminMT.WebApplication
    ```
...
2. Realizar a migração do zero
    ...
    Para criar a migração inicial do zero, é necessário excluir a pasta "Migrations" que consta no projeto "CeciAdminMT.Infra.Data". A nova migração pode ser criada com o comando "dotnet ef migrations add [NOME_MIGRACAO]". Por conta da estrutura do projeto é necessário especificar em qual projeto está a classe de contexto e qual é o projeto de inicialização:

    ```
    dotnet ef migrations add InitialMigration --project CeciAdminMT.Infra.Data --startup-project CeciAdminMT.WebApplication
    ```
...
```
dotnet ef migrations add [NOME_MIGRACAO] --project CeciAdminMT.Infra.Data --startup-project CeciAdminMT.WebApplication
```
...
Navegue até o projeto WebApplication "CeciAdminMT.WebApplication" e execute o comando "dotnet run": 

```
PS ...\CeciAdminMT\CeciAdminMT.WebApplication> dotnet run
```
...
Navegue até o projeto de testes "CeciAdminMT.Test" e execute o comando "dotnet test": 

```
PS ...\CeciAdminMT\CeciAdminMT.Test> dotnet test
```
...
O arquivo "DockerFile", já está contemplado nessa versão e está disponível na raiz da solução, em alguns casos o arquivo é armazenado na raiz do projeto WebApplication, mais como o projeto CeciAdminMT possui mais de um projeto na solução e são dependências de projetos, a publicação no Heroku só foi concluída com essa organização. Vale ressaltar que essa foi uma configuração na qual foi possível publicar o projeto CeciAdminMT no Heroku, não necessáriamente é uma regra para o caminho do arquivo "DockerFile".
